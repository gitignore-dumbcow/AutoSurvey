...
class SurveyContext:
    def __init__(self, entry, logger, submit):
        ...
        self.driver.get("https://survey.alchemer.com/s3/8332127/C9954-DCSI-2025-June-2025")
...
def fill_contact_info(ctx):
    ctx.wait.until(EC.presence_of_element_located((By.ID, "sgE-8332127-29-53-11062"))).send_keys(ctx.entry.ten_dv)
    ctx.wait.until(EC.presence_of_element_located((By.ID, "sgE-8332127-29-53-11064"))).send_keys(ctx.entry.sdt)
    ctx.logger.log(f"[{now()}] - O - Tên: {ctx.entry.ten_dv}, SĐT: {ctx.entry.sdt}")

def fill_q2_feedback(ctx):
    try:
        # Q2 Hài lòng
        box1 = ctx.driver.find_element(By.ID, "sgE-8332127-66-114-element")
        box1.clear()
        ...
        box2 = ctx.driver.find_element(By.ID, "sgE-8332127-66-118-element")
        box2.clear()
...
def fill_survey(entry, logger, submit=False):
    ...
    try:
        ...
        ctx.driver.find_element(By.ID, "sgE-8332127-1-2-10001").send_keys(entry.msbch)
        logger.log(f"[{now()}] - O - Mã BCH: {entry.msbch}")
        ctx.driver.find_element(By.ID, "sgE-8332127-1-2-10002").send_keys("N24255")
        logger.log(f"[{now()}] - O - Mã số PVV: N24255")
        ctx.driver.find_element(By.ID, "sgE-8332127-1-2-10003").send_keys("Nguyễn Kiều Anh")
        logger.log(f"[{now()}] - O - Họ tên PVV: Nguyễn Kiều Anh")
        click_next_button(ctx)

        click_element(ctx, "sgE-8332127-13-17-10028")  # Khu vực
        logger.log(f"[{now()}] - O - Khu vực: HA NOI")
        click_next_button(ctx)

        ctx.wait.until(EC.presence_of_element_located((By.ID, "sgE-8332127-31-59-element"))).send_keys(entry.s4_head)
        logger.log(f"[{now()}] - O - Mã đại lý: {entry.s4_head}")
        click_next_button(ctx)
        click_next_button(ctx)

        gender_id = "sgE-8332127-40-73-11935" if entry.gioi_tinh.lower() in ["nữ", "nu"] else "sgE-8332127-40-73-11934"
        click_element(ctx, gender_id)
        logger.log(f"[{now()}] - O - Giới tính: {entry.gioi_tinh}")
        click_next_button(ctx)

        click_element(ctx, "sgE-8332127-41-74-11936")
        logger.log(f"[{now()}] - O - Đúng thông tin")
        click_next_button(ctx)
        
        click_element(ctx, "sgE-8332127-42-76-11943")
        logger.log(f"[{now()}] - O - Đồng ý")
        click_next_button(ctx)

        
        click_element(ctx, "sgE-8332127-44-78-11945")
        logger.log(f"[{now()}] - O - Đồng ý")
        
        click_next_button(ctx)

        click_element(ctx, "sgE-8332127-45-80-11947")
        logger.log(f"[{now()}] - O - Có")
        click_next_button(ctx)

        click_element(ctx, "sgE-8332127-46-82-11951")
        logger.log(f"[{now()}] - O - Đúng hệ thống")
        click_next_button(ctx)

        Select(ctx.driver.find_element(By.ID, "sgE-8332127-47-110-element")).select_by_visible_text(entry.tinh_song)
        click_element(ctx, "sgE-8332127-47-85-11955")
        logger.log(f"[{now()}] - O - Đúng hệ thống")
        click_next_button(ctx)
        ...
        select_yes_no_maybe(ctx, entry.q5, "sgE-8332127-53-96-12010", "sgE-8332127-53-96-12011", "sgE-8332127-53-96-12062", "Q5")
        select_rating(ctx, entry.q6, "Q6")
        select_yes_no(ctx, entry.q7, "sgE-8332127-55-97-12012", "sgE-8332127-55-97-12013", "Q7")
        select_rating(ctx, entry.q8, "Q8")
        select_yes_no(ctx, entry.q9, "sgE-8332127-59-101-12029", "sgE-8332127-59-101-12030", "Q9")
        select_rating(ctx, entry.q10, "Q10")
        select_yes_no(ctx, entry.q12, "sgE-8332127-61-105-12046", "sgE-8332127-61-105-12047", "Q12")
        if entry.q12.strip().lower() in ["có", "yes", "1"]:
            select_rating(ctx, entry.q13, "Q13")
        select_yes_no_maybe(ctx, entry.q15, "sgE-8332127-65-122-12153", "sgE-8332127-65-122-12154", "sgE-8332127-65-122-12155", "Q15")
